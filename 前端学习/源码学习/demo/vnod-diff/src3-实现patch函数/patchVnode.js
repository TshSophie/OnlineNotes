...
export default function patchVnode(oldVnode, newVnode) {
  console.log('1. 判断新旧 vnode 是否是同一个对象')
  // 1. 判断新旧 vnode 是否是同一个对象
  if (oldVnode === newVnode) return;
  console.log('2. 判断 newVndoe 有没有 text 属性')
  // 2. 判断 newVndoe 有没有 text 属性
  if (newVnode.text !== undefined &&(newVnode.children === undefined || newVnode.children.length === 0)) {
    // newVnode 有 text 属性
    console.log('————【newVnode 有 text 属性】————')
    console.log('2.1 判断 newVnode 与 oldVnode 的 text 属性是否相同')
    ...
    console.log('————【newVnode 没有text属性 有children属性】————')
    console.log('     2.2 判断 oldVnode 有没有 children 属性')
    // 2.2 判断 oldVnode 有没有 children 属性
    if (oldVnode.children !== undefined && oldVnode.children.length > 0) {
      // oldVnode有children属性 最复杂的情况，新老节点都有children
      console.log('       oldVnode有children属性 最复杂的情况，新老节点都有children')
      updateChildren(oldVnode.elm, oldVnode.children, newVnode.children);
    } else {
      // oldVnode没有children属性 说明有text;  newVnode有children属性
      console.log('       oldVnode没有children属性 说明有text;  newVnode有children属性')
      // 清空oldVnode的内容
