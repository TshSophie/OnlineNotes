...
**函数节流**是指预先设定一个执行周期，当调用动作的时刻大于等于执行周期则执行该动作，然后进入下一个新周期。简单来说，就是限制一个方法在一定时间内执行一次。

**防抖节流的区别**：
防抖的目的是为了让函数虽然会多次执行，但是有效代码只执行一次。
...
### 实际应用
- 搜索框搜索输入。只需用户最后一次输入完，再发送请求。输入结束后n秒才进行搜索请求，n秒内又输入的内容，就重新计时
- 手机号、邮箱验证输入检测
...
当持续触发事件时，保证一段时间内，只调用一次事件处理函数。按照设定的频率执行。
